...
<head>
    ...
    <script type="text/javascript">
        ...
        OneSignal.push(["init", {
            appId: "eceb4836-43bd-4b60-9ee7-55b812f11792",
        ...
        OneSignal.push(function() {
            ...
            if (isPushSupported) {
                ...
                console.log('onesignal is supported');
                OneSignal.push(function() {
                    OneSignal.isPushNotificationsEnabled()
                        .then(function(isPushEnabled) {
                            console.log("Site Notification Subscribtion:", isPushEnabled);
                            if (isPushEnabled) {
                                OneSignal.getUserId().then(function(userId) {
                                    console.log("OneSignal User ID:", userId);
                                    sessionStorage.setItem('onesignal_pId', userId);
                                    // console.log("{{ Auth::user()->device_token }}");
                                    currentTok = '{{ Auth::user()->device_token }}';

                                    $.ajax({
                                        type: "POST",
                                        data: {
                                            'user_id': '{{ Auth::user()->id }}',
                                            'one_signal_pId': userId,
                                            'device_token': currentTok,
                                            'api_token': '{!! auth()->user()->api_token !!}'
                                        },
                                        dataType: "json",
                                        url: "{!! url('api/update_player_id') !!}",
                                        success: function(data) {
                                            // console.log(data);
                                        },
                                        error: function(err) {
                                            console.log('failed to update playerID: ');
                                            console.log(err);
                                        }
                                    });
                                });
                ...
                console.log('onesignal is not supported');
        ...
        //         /*  OneSignal.push(function() {
        //              OneSignal.permissionPromptDisplay();
        //          }); */
    ...
    <script>
        ...
        var pusher = new Pusher('27da49c0e8dceaf06605', {
            cluster: 'mt1'
        });

        var channel = pusher.subscribe('import-update-status');
        channel.bind('progress-changed', function(data) {
            var progressData = Math.ceil(data['importProgress']) + '%';
            var l1 = $('#loaderBar1');
            var bar2 = $('#bar2');
            var l2 = $('#loaderBar2');
            ...
            l1.hide(1000);
            l2.show(1000);
            bar2.width(progressData)
            bar2.html(progressData + ' Completed');
        });
    </script>
    @endif
</head>
