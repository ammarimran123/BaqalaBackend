<!-- Id Field -->
<div class="form-group row col-6">
  {!! Form::label('id', 'Id:', ['class' => 'col-3 control-label text-right']) !!}
  <div class="col-9">
    <p>{!! $market->id !!}</p>
  </div>
</div>

<!-- Name Field -->
<div class="form-group row col-6">
  {!! Form::label('name', 'Name:', ['class' => 'col-3 control-label text-right']) !!}
  <div class="col-9">
    <p>{!! $market->name !!}</p>
  </div>
</div>



<!-- Description Field -->
<div class="form-group row col-6">
  {!! Form::label('description', 'Description:', ['class' => 'col-3 control-label text-right']) !!}
  <div class="col-9">
    <p>{!! $market->description !!}</p>
  </div>
</div>

<!-- Image Field -->
<div class="form-group row col-6">
  {!! Form::label('image', 'Image:', ['class' => 'col-3 control-label text-right']) !!}
  <div class="col-9">
    <p>{!! $market->image !!}</p>
  </div>
</div>

<!-- Address Field -->
<div class="form-group row col-6">
  {!! Form::label('address', 'Address:', ['class' => 'col-3 control-label text-right']) !!}
  <div class="col-9">
    <p>{!! $market->address !!}</p>
  </div>
</div>

<!-- Latitude Field -->
<div class="form-group row col-6">
  {!! Form::label('latitude', 'Latitude:', ['class' => 'col-3 control-label text-right']) !!}
  <div class="col-9">
    <p>{!! $market->latitude !!}</p>
  </div>
</div>

<!-- Longitude Field -->
<div class="form-group row col-6">
  {!! Form::label('longitude', 'Longitude:', ['class' => 'col-3 control-label text-right']) !!}
  <div class="col-9">
    <p>{!! $market->longitude !!}</p>
  </div>
</div>

<!-- Min Order Amount -->
<div class="form-group row col-6">
  {!! Form::label('min_order_amount', 'Min Order Amount:', ['class' => 'col-3 control-label text-right']) !!}
  <div class="col-9">
    <p>{!! $market->min_order_amount !!}</p>
  </div>
</div>

<!-- Start Time -->
<div class="form-group row col-6">
  {!! Form::label('start_time', 'Start Time:', ['class' => 'col-3 control-label text-right']) !!}
  <div class="col-9">
    <p>{!! date('HH:MM', $market->start_time) !!}</p>
  </div>
</div>

<!-- End Time -->
<div class="form-group row col-6">
  {!! Form::label('end_time', 'End Time:', ['class' => 'col-3 control-label text-right']) !!}
  <div class="col-9">
    <p>{!! $market->end_time !!}</p>
  </div>
</div>

<!-- Phone Field -->
<div class="form-group row col-6">
  {!! Form::label('phone', 'Phone:', ['class' => 'col-3 control-label text-right']) !!}
  <div class="col-9">
    <p>{!! $market->phone !!}</p>
  </div>
</div>

<!-- Mobile Field -->
<div class="form-group row col-6">
  {!! Form::label('mobile', 'Mobile:', ['class' => 'col-3 control-label text-right']) !!}
  <div class="col-9">
    <p>{!! $market->mobile !!}</p>
  </div>
</div>

<!-- Information Field -->
<div class="form-group row col-6">
  {!! Form::label('information', 'Information:', ['class' => 'col-3 control-label text-right']) !!}
  <div class="col-9">
    <p>{!! $market->information !!}</p>
  </div>
</div>

<!-- Created At Field -->
<div class="form-group row col-6">
  {!! Form::label('created_at', 'Created At:', ['class' => 'col-3 control-label text-right']) !!}
  <div class="col-9">
    <p>{!! $market->created_at !!}</p>
  </div>
</div>

<!-- Updated At Field -->
<div class="form-group row col-6">
  {!! Form::label('updated_at', 'Updated At:', ['class' => 'col-3 control-label text-right']) !!}
  <div class="col-9">
    <p>{!! $market->updated_at !!}</p>
  </div>
</div>

