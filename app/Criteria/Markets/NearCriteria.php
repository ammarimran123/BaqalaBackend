    ...
    public function apply($model, RepositoryInterface $repository)
    {
        if ($this->request->has(['myLon', 'myLat', 'areaLon', 'areaLat'])) {
            ...
            return $model->select(DB::raw("SQRT(
                ...
                // ->where('delivery_range', '>=', SQRT(POW(69.1 * (0 - $myLat), 2) + POW(69.1 * ($myLon - 0) * COS(0 / 57.3), 2)))
                ->where('delivery_range', '>=', DB::raw("SQRT(POW(69.1 * (latitude - $myLat), 2) + POW(69.1 * ($myLon - longitude) * COS(latitude / 57.3), 2))"))
                ->orderBy('closed')
                ->orderBy('area');
        } else {
