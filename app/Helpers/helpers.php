...
function getMediaColumn($mediaModel, $mediaCollectionName = '', $optionClass = '', $mediaThumbnail = 'icon')
{
    ...
    if ($mediaModel->hasMedia($mediaCollectionName)) {
        ...
        $mediaData = DB::table('media')
            ->where('model_id', $mediaModel->id)
            ->get();

        Storage::put('queryResult.txt', $mediaData);
        if (Storage::disk('s3Public')->exists('public/' . $mediaData[0]->id . '/conversions/')) {
            $url = Storage::disk('s3Public')->url('public/' . $mediaData[0]->id . '/' . $mediaData[0]->file_name);
            Storage::put('url.txt', $url);
            return "<img class='" . $optionClass . "' style='width:50px' src='" . $url . "' alt='image_default'>";
        } else {
...
//updating specialoffer if discounted price (used in product controller)
function updateSpecialOffer($market_id)
{
