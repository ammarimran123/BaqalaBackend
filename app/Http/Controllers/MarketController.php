...
//   dd($market);
    ...
    public function deleteAllproducts($id)
    {
        ...
        // if ($market) {
        $products=Product::whereIn('market_id', $ids)->delete();
        if($products==0)
        {
            updateSpecialOffer($id);
            Flash::error('There are no products to be deleted!');
            return redirect(route('markets.index'));
        }
        else
        {
            updateSpecialOffer($id);
            Flash::success(__('All products deleted successfully.', ['operator' => __('lang.market')]));
            return redirect(route('markets.index')); 
        }
    }
}
