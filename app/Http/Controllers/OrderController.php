...
class OrderController extends Controller
{
    ...
    public function show(ProductOrderDataTable $productOrderDataTable, $id)
    {
        ...
        // $service_total = $total * $service_fee->service_fees;
        // $service_total = $service_total / 100;
        $total = $total + $service_fee;
        $productOrderDataTable->id = $id;

        return $productOrderDataTable->render('orders.show', ["order" => $order, "total" => $total, "subtotal" => $subtotal, "taxAmount" => $taxAmount, "serviceFee" => $service_fee]);
    }
    /**
    ...
    public function update($id, UpdateOrderRequest $request)
    {
        ...
        $input = $request->all();
        Storage::put('outPut.txt', ' Req:' . $request);

        $customFields = $this->customFieldRepository->findByField('custom_field_model', $this->orderRepository->model());
        try {

            $order = $this->orderRepository->update($input, $id);
            Storage::put('outPut.txt', ' Req:' . $order->user);
